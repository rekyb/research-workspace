...
You are starting a new secondary desk-research effort.
...
2. **Establish and confirm the goal (required).** The research goal drives everything
   ...
   - If the research topic/query already states a clear goal (what we want to learn *and* why —
...
7. **Confirm** to the user: folder created, the confirmed goal, the approved plan,
   and that capture happens next via the browser tools.
