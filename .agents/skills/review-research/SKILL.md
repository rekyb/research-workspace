...
2. **Gather the ground truth.** Read `SYNTHESIS.md` in full, plus the research
   ...
   If the user passed a specific focus/argument, weight the review toward it but still
...
3. **Run the three personas as separate subagents, chained so they can cross-talk.**
   ...
   with a subagent (using `general-purpose` / `research` or a subagent configured with the same rules), handing it its spec file plus the ground
