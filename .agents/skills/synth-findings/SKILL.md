...
3. **Write `SYNTHESIS.md`** in the research folder. Structure it as a list of
   **features** distilled across the benchmarked platforms. Lead with a short
   ...
   3. **Key findings** — what we learned observing it. The analysis under key findings must follow the logic: what the user sees, what the user does, and what the system does (written as paragraphs/text flowing in this order, rather than a table). Cite the platform(s) and evidence. **CRITICAL:** When citing a screenshot or GIF, embed it directly using relative markdown syntax (e.g., `![description](../platforms/<platform>/screenshots/filename.png)` or `![flow](../platforms/<platform>/flow.gif)`).
...
4. **Principal Researcher QA pass (quality gate — before `review-research`).**
   Dispatch the Principal Researcher as a subagent (using `general-purpose` / `research` or a subagent configured with the same rules) in
...
5. **Optional docx.** If requested or if the args contain `--docx`, run:
